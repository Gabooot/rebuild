...
func _physics_process(_delta):
	...
	if server.is_connection_available():
		...
		peers.append(peer)
		game.spawn(Vector3(0, 0.5, 10), num_players)
...
func send_positions() -> void:
	for i in range(0, peers.size()):
		...
		var origin = tank.global_transform.origin
		...
		#print("quat to basis: ", Basis(quaternion))
		#var position = game.get_node(str(i) + "/tank").global_position
		#var rotation = game.get_node(str(i) + "/tank").rotation
